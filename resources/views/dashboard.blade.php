                ...
                <div class="col-md-2 responsive-flex">
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        <a href="{{ url('return-order') }}"
                            class="btn btn-primary btn-sm btn-block @if (request()->is('return-order')) active @endif">Return
                            Orders</a>
                        <a href="{{ url('profile') }}"
                                                    ...
                                                    @else
                                                        @foreach ($orders as $order)
                                                            <tr class="cell-1">
                                                                ...
                                                                <td><span
                                                                        class="badge badge-warning badge-pill">{{$order->status }}
                                                                    </span>
                                                                    @if($order->return_reason != null)
                                                                    
                                                                    <span
                                                                        class="badge badge-danger badge-pill">Return Request
                                                                    </span>
                                                                    @endif
                                                                </td>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <a href="{{ url('order-return/' . $order->id) }}"
                                                                        ...
                                                                        @if ($order->status != 'delivared') disabled @endif><i
                            ...
                            <div class="pricing">
                                <div class="row">
                                    <div class="col-md-4" style="padding-left: 0px">
                                        ...
                                        <address>
                                            {{ strtoupper($order->name) }}<br>{{ strtoupper($order->address) }}<br>{{ $order->area->post_office_name }}<br>{{ $order->city->city_name }}<br>{{ $order->division->division_name }}-{{ $order->post_code }}<br>Bangladesh
                                        </address>
                                    </div>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-9" style="padding-left: 0px">
                                                ...
                                                @foreach ($orderitems as $orderitem)
                                                    <div id="name">{{ $orderitem->product->product_name_en }}
                                                        ({{ $orderitem->qty }})
                                                    </div>
                                                @endforeach
                                                <div id="name">Tax</div>
                                    ...
                                    <ul id="progressbar">
                                        <li class="step0 @if (
                                            $order->status == 'pending' ||
                                                ...
                                                $order->status == 'delivared') active @endif "
                                            id="step1">Ordered</li>
                                        <li class="step0 text-center @if (
                                            $order->status == 'confirmed' ||
                                                $order->status == 'processing' ||
                                                $order->status == 'picked' ||
                                                $order->status == 'shipped' ||
                                                $order->status == 'delivared') active @endif"
                                            id="step2">Confirmed</li>
                                        <li class="step0 text-right @if (
                                            $order->status == 'processing' ||
                                                $order->status == 'picked' ||
                                                $order->status == 'shipped' ||
                                                $order->status == 'delivared') active @endif"
                                            id="step3">Processing</li>
                                        <li class="step0 text-right @if ($order->status == 'picked' || $order->status == 'shipped' || $order->status == 'delivared') active @endif"
                                            id="step4">Picked</li>
                                        <li class="step0 text-right @if ($order->status == 'shipped' || $order->status == 'delivared') active @endif"
                                            id="step5">Shipped</li>
                                        <li class="step0 text-right @if ($order->status == 'delivared') active @endif"
                                            id="step6">Delivered</li>
                                    </ul>
                                </div>
                            @endif
                            @if (request()->is('order-return/*') && $order->status == 'delivared' && $order->return_reason == null)
                                <form action="{{ url('order-return/' . $order->id) }}" method="post">
                                    @csrf
                                    <div class="form-group">
                                        <label for="returnReason">Reason for Return<span class="text-danger">
                                                *</span></label>
                                        <textarea class="form-control" id="returnReason" rows="3" placeholder="Enter the reason for return"
